...
Handled as follows. `S2_v2.R` is **unchanged**. `R/01_estimates.R` produces both
...
number in `results/revision/` uses.
...
(`outputs/legacy_vs_corrected_specification.csv`):
...
sound, but they should not be quoted to four digits. `results/revision/` uses the
