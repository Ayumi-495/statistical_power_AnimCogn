...
The claims below are enumerated BY HAND from the manuscript text rather than parsed out
of it, because a parser would silently skip anything it failed to recognise and report
success. Each entry records the sentence the number appears in, the figure exactly as
written there, and where it comes from. A claim is correct when the stored value,
converted to the units the sentence uses and rounded to the number of digits the sentence
shows, equals what is written.

This list is therefore also a machine-readable record of every number the manuscript
asserts, independent of the manuscript file itself: nothing here is read from the
manuscript at run time.

WHEN THIS DISAGREES, THE DOCUMENT IS WRONG, NOT THE CSV.

Run: python3 revision/R/analyse/21_audit_manuscript_claims.py
"""
import csv
import os
import sys
...
REV = os.path.join(REVISION, "results")
...
def main():
    ...
    if fails:
        ...
        print("\nThe manuscript text is wrong, not the CSV. Fix the sentence, not the result file.")
        return 1
