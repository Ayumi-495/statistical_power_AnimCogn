# revision/R/analyse/00_revision_functions.R --------------------------------------------
...
# `S2_v2.R` is never sourced, edited or run by anything in revision/R/analyse/.
#
# Two deliberate differences from revision/R/reproduce/00_setup.R, both documented here because they
# change reported numbers:
#
#   1. Type M is computed in CLOSED FORM, not by Monte Carlo. `S2_v2.R` and
#      revision/R/reproduce/00_setup.R draw 10,000 normal deviates per unit, which makes results
...
#      N = 200,000. Every Type M in revision/results/ is closed-form.
#   2. Data loading and the optimizer table are REUSED from revision/R/reproduce/00_setup.R rather
...
source(here::here("revision", "R", "reproduce", "00_setup.R"))   # load_datasets(), dataset_index(), fit_rma(), optimizer_for()

REV_OUT <- here::here("revision", "results")
# Chaining artifacts (fitted objects passed from one script to the next) live in a
# separate, gitignored subdirectory so that revision/results/ contains only the
...
# empirically it does: see revision/results/reversal_counts.csv.
#
...
# revision/results/README.md. Note this also means 5,740 is a count of effect-size
