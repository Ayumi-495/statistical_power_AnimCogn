...
files <- tibble::tribble(
  ...
  "loo_influence.csv", "Leave-one-model-out influence on the meta-analysis-level summaries: all 48 models by 4 specifications by 3 metrics by both weightings. Companion to leave_one_paper_out.csv, which removes one source paper at a time together with every model it contributed. Single-model leverage is largely a property of the weighting: under weighting by effect-size count one model moves a summary by more than 20%, and under equal weighting no model moves any summary by more than 10%.",
